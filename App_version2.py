...
def show():
    ...
    elif method=='SQL Queries(ON VPN)':
        with ThreadPoolExecutor() as executor:
            try:
                folder_select.config(text=" ",fg='blue')
                for widget in bottomframe.winfo_children():
...
bottomframe_bb.pack(side=BOTTOM,padx=(20,20),pady=(5,10))

bottomframe_L=Frame(bottomframe_2,bg='#F5F5F5')
bottomframe_L.pack(side=LEFT,pady=(10,10),padx=(10,10))

bottomframe_R=Frame(bottomframe_2,bg='#F5F5F5')
bottomframe_R.pack(side=RIGHT,pady=(20,10),padx=(0,10))
...
# initial menu text
clicked.set("Step 2: Choose a Process to Proceed")

# Create Dropdown menu
drop = OptionMenu(bottomframe_L, clicked, *options)
...
button=Button(bottomframe_R, text="Step 3: Compare", command= lambda: threading.Thread(target=buttons).start())
